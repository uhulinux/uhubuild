...
#	ver="$(urllist "https://mirrors.gethosted.online/gnome/sources/$project/" | grep -E '^[0-9.]+['$reg'](\.[0-9.])?$' | sort -V | tail -n 1)"
...
function u2dmate {
	...
	for ver in $(urllist "https://pub.mate-desktop.org/releases/" | grep -E '^[0-9]+\.[0-9]*['$reg'](\.[0-9.])?$' | sort -rV);do
        version=$(urllist https://pub.mate-desktop.org/releases/"$ver/" | splitver '^'$project'-([0-9.]+).tar.(gz|bz2|xz)$' | sort -V | tail -n 1)
        if [ "$version" ];then
            echo $version
            exit 0
        fi
